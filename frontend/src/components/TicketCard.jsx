...
export default function TicketCard({ticket,onView,onEdit,onDelete}){return <Card variant="outlined" onClick={()=>onView(ticket)} sx={{height:'100%',display:'flex',flexDirection:'column',cursor:'pointer'}}>
 ...
 <CardActions sx={{justifyContent:'flex-end'}} onClick={event=>event.stopPropagation()}><IconButton aria-label="edit" onClick={()=>onEdit(ticket)}><EditOutlined/></IconButton><IconButton aria-label="delete" color="error" onClick={()=>onDelete(ticket.id)}><DeleteOutline/></IconButton></CardActions>
 </Card>}
